...
We also use the same process calculus defined before:

\begin{align*}
P ::&= 0 \mid \mu X. P \mid X \\
&\mid p \rhd \{ l_i: P_i \}_{i \in I} \\
&\mid p \lhd l. P \\
&\mid \texttt{if e then } P \texttt{ else } P\\\\
\mathcal{M} ::&= p :: P \\
&\mid \mathcal{M} \mid \mathcal{M}
\end{align*}

The typing judgement for a multiparty session with respect
to the webservice global type builds upon the \textsc{[MTy]}
rule.


\subsection*{Codegen}
\texttt{codegen :: $\mathcal{M}$ $\times$ Role 
$\to$ $\mathcal{M}$}
...
\begin{align*}
&codegen~(p :: q \lhd l_j. P 
\mid q :: p \rhd \{ l_i: Q_i \}_{i \in I} 
\mid s :: S 
\mid \mathcal{M})~s \\
=~&codegen~(p :: s \lhd l. P 
\mid q :: s \rhd \{l_i: Q_i\}_{i \in I}
\mid s :: p \rhd )\\
&where  \\
\dots
\end{align*}

\subsubsection*{Trivial rules}

\subsection*{Encoding}

\subsection*{Preservation}
Modifying the process through code generation preserves
well-typedness of the session.

\paragraph{To show:} let $G_{WS} = (G, p)$. 
If $\mathcal{M}$ is well-typed with respect to $G_{WS}$,
then \texttt{codegen($\mathcal{M},~p$)} is well-typed
with respect to $\llbracket G_{WS} \rrbracket$
