...
This motivates the concept of \term{subtypes}, which allows a process to be typed by its ``supertype'' when required. $\leqslant$ defines the subtyping relation: $S \leqslant S'$ reads \textit{$S$ is a subtype of $S'$}, and is defined coinductively. 
...
We also introduce the \rulename{Ty-Sub} to incorporate the subtyping relation into the typing judgement.
