...
channel linearity.
Our design goal is to enforce linearity statically through the generated APIs.
...
keyboard) which interacts with some DOM elements.
We could pass a
...
role identifier:
it opens a WebSocket connection to the server, keeps track of the current
...
customise how the state is rendered and inject program logic into state
...
A limitation of our browser-side session type encoding is
that channel resources are guaranteed to be used \textit{at most once} as opposed to
\textit{exactly once}.
...
unused states.
Equally, our approach does not prevent a client closing the browser, which
